...
const int N = 5005, inf = 1e18, LG = 18; // change here
...
int f(int i, int j) {
  ...
  int lc = lcp[i][j];
  int mn = j - i;
  if (lc >= j - i and j + mn <= n) {
    ans = max(ans, 1 + f(j, j + mn + 1));
  }
  else if (i + lc <= n and j + lc <= n) {
    if (s[j + lc] <= s[i + lc]) return ans;
    ans = max(ans, 1 + f(j, j + lc + 1));
  }
  return ans;
}
...
void print(int i, int j) {
  ...
  int mn = j - i;
  if (lc >= j - i and j + mn <= n) {
    int ans2 = 1 + f(j, j + mn + 1);
    if (ans == ans2) {
      // cout << "hrere: " << j << ' ' << j + mn << '\n';
      cout << '\n';
      for (int k = j; k <= j + mn; k++) {
        cout << s[k];
      }
      print(j, j + mn + 1);
      return;
    ...
    if (s[j + lc] <= s[i + lc]) return;
    int ans3 = 1 + f(j, j + lc + 1);
    if (ans == ans3) {
      cout << '\n';
      for (int k = j; k <= j + lc; k++) {
        cout << s[k];
      }
      print(j, j + lc + 1);
      return;

...
void solve() {
  ...
  auto ok = [&](int i, int j, int k) {
    ...
    sort(v.begin(), v.end());
    i = v[0], j = v[1], k = v[2];
    if (i < n and j > n and j <= n + m and k > n + m) return true;
    return false;
  };

  int ans = 0;
  for (int i = 0; i + 2 < n; i++) {
    int lcp = min(sa.get_lcp(sa.sa[i], sa.sa[i + 1]), sa.get_lcp(sa.sa[i + 1], sa.sa[i + 2]));
    if (ok(sa.sa[i], sa.sa[i + 1], sa.sa[i + 2])) {
      ans = max(ans, lcp);
    }
  }

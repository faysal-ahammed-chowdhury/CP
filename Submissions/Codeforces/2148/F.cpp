...
int lcp(int i, int j, int x, int y, int idx1, int idx2) {
  int len1 = j - i + 1, len2 = y - x + 1;
  int l = 1, r = min(len1, len2), ans = 0;
  while (l <= r) {
    int mid = (l + r) >> 1;
    if (hs[idx1].get_hash(i, i + mid - 1) == hs[idx2].get_hash(x, x + mid - 1)) {
      ans = mid;
      l = mid + 1;
    }
    else {
...
int compare(int i, int j, int x, int y, int idx1, int idx2) {
  int common_prefix = lcp(i, j, x, y, idx1, idx2);
  int len1 = j - i + 1, len2 = y - x + 1;
  if (common_prefix == len1 and len1 == len2) return 0;
  else if (common_prefix == len1) return -1;
  else if (common_prefix == len2) return 1;
  else return (a[idx1][i + common_prefix - 1] < a[idx2][x + common_prefix - 1] ? -1 : 1);
}
...
void solve() {
  ...
  for (int j = 1; j <= m; j++) {
    ...
    for (int i = 1; i <= n; i++) {
      if (a[i].size() >= j) {
        if (idx == -1) idx = i;
          ...
          int x = compare(j, a[idx].size(), j, a[i].size(), idx, i);
          if (x == 1) {
            idx = i;
          }
        } 
    ...
    for (int k = j; k <= a[idx].size(); k++) {
      ans.push_back(a[idx][k - 1]);
    }
    j = a[idx].size();
  }

...
struct SuffixArray {
  ...
  bool ok(int idx, int add, char c) {
    return ((sa[idx] + add) < n and s[sa[idx] + add] >= c);
  }
  bool ok2(int idx, int add, char c) {
    return ((sa[idx] + add) >= n or ((sa[idx] + add) < n and s[sa[idx] + add] <= c));
  }

  vector<int> get_ans(string &t) {
    int lo = 0, hi = n - 1;
    vector<int> ans;
    bool flag = true;
    for (int i = 0; i < t.size(); i++) {
      int l = lo, r = hi, cur_ans = -1;
      char c = t[i];
      while (l <= r) {
        int mid = (l + r) / 2;
        if (ok(mid, i, c)) {
      ...
      // cout << i << ' ' << c << ' ' << cur_ans1 << ' ' << cur_ans2 << '\n';

      if (cur_ans1 <= cur_ans2 and cur_ans1 != -1 and cur_ans2 != -1) {
        if (flag) ans.push_back(1);
        else ans.push_back(0);
        flag = true;
        lo = cur_ans1, hi = cur_ans2;
      }
      else {
        if (flag == false) assert(false);
        flag = false;
        if (t[i] == '0') t[i] = '1';
        else t[i] = '0';
        i--;
